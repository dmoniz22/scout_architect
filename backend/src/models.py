...
from sqlalchemy import create_engine, Column, Integer, String, Text, Boolean, Date, DateTime, ForeignKey, ARRAY, Numeric, JSON
...
class OASSkill(Base):
    ...
    level1_desc = Column(Text)
    level2_desc = Column(Text)
    level3_desc = Column(Text)
    level4_desc = Column(Text)
...
class TermPlan(Base):
    ...
    meetings = relationship("MeetingPlan", back_populates="term_plan", cascade="all, delete-orphan")
